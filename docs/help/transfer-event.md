---
title: "Transferring event ownership"
meta_description: "How to hand over a Cloomba event to another user — the request, the 7-day window, and what changes when it is accepted."
...
1. Open the event you want to transfer.
2. Go to **Edit** → **Transfer ownership**.
3. Enter the username of the new owner.
4. Send the request.

The recipient sees a notification and gets an email with a review link. They have **7 days** to accept; otherwise the request expires.
...
You can't transfer an event that is **upcoming and has paid tickets**. Sales are routing to your Stripe account; switching ownership mid-flight would break payouts. Either wait until the event has passed, or switch the event to free first.
